...
def eval_all(expressions, env):
    ...
    '''
    I have to change my original correct version to fit the requirements of Q19
    curr = expressions
    ...
    if expressions is not nil:
        ...
        while curr.rest is not nil:
            ...
            curr = curr.rest
        # Tail context 5: the last sub-expression in [begin]
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        curr1 = formals
        curr2 = vals
        while curr1 is not nil and curr2 is not nil:
            newFrame.define(curr1.first, curr2.first)
            curr1 = curr1.rest
            curr2 = curr2.rest
        if curr1 is nil and curr2 is nil:
            return newFrame
        raise SchemeError('The number of argument values does not match with formal parameters')
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('The arguments does not fit the given built-in function!')
        # END PROBLEM 3
...
def do_if_form(expressions, env):
    ...
    # Tail context 2: sub-expression 2 & 3 in [if]
...
def do_and_form(expressions, env):
    ...
    curr = expressions
    '''
    I have to change my original corect version to fit the requirements of Q19
    while curr is not nil:
        value = scheme_eval(curr.first, env)
    ...
    while curr is not nil:
        if curr.rest is nil:
            # Tail context 4: the last sub-expression in [and]
            value = scheme_eval(curr.first, env, True)
        else:
            value = scheme_eval(curr.first, env)
        if is_false_primitive(value):
            return False
...
def do_or_form(expressions, env):
    ...
    while curr is not nil:
        if curr.rest is nil:
            # Tail context 4: the last sub-expression in [or]
            value = scheme_eval(curr.first, env, True)
        else:
            value = scheme_eval(curr.first, env)
        if is_true_primitive(value):
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        formals = target.rest
        body = expressions.rest
        Frame.define(env, name, MacroProcedure(formals, body, env))
        return name
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
SPECIAL_FORMS['define-macro'] = do_define_macro
