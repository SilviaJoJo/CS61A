...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            if isinstance(place.ant, ContainerAnt) or isinstance(self, ContainerAnt):
                if not isinstance(self, ContainerAnt) and not place.ant.contained_ant:
                    place.ant.contained_ant = self
                elif not isinstance(place.ant, ContainerAnt) and not self.contained_ant:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        targetPlace = self.place
        entrances = 0
        test = None
        while (not test or entrances <= self.min_range) and entrances <= self.max_range:
            if targetPlace is beehive:
                break
            elif entrances >= self.min_range and entrances <= self.max_range:
                test = rANTdom_else_none(targetPlace.bees)
            targetPlace = targetPlace.entrance
            entrances += 1
        return test
        # END Problem 3 and 4
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.identity:
            ...
            while curr:
                if curr.ant:
                    if curr.ant.doubled == False:
                        curr.ant.damage *= 2
                        curr.ant.doubled = True
                    if isinstance(curr.ant, ContainerAnt) and curr.ant.contained_ant:
                        if curr.ant.contained_ant.doubled == False:
...
class Bee(Insect):
    ...
    def __init__(self, armor, place=None):
        ...
        self.direction = 1 # represents that the bee is moving leftwards
        self.scared = False
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        while curr is not beehive:
            if curr.ant and not isinstance(curr.ant, LaserAnt):
                targets[curr.ant] = distance
