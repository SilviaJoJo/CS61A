...
def make_glookup(class_assignments):
    ...
    "*** YOUR CODE HERE ***"
    curr_assignments = {}
    def f(assignment, grade):
        nonlocal curr_assignments
        def g():
            curr_assignments[assignment] = grade
            currScore = sum(list(curr_assignments.values()))
            totalScore = sum([class_assignments[key] for key in curr_assignments.keys()])
