...
class VendingMachine:
    ...
    def restock(self, amount):
        self.amount += amount
        return 'Current {0} stock: {1}'.format(self.stock, self.amount)
...
def store_digits(n):
    ...
    remainder = n % 10
    quotient = n // 10
    ans = store_digits(quotient)
    curr = ans
    while curr.rest is not Link.empty:
        curr = curr.rest
    curr.rest = store_digits(remainder)
    return ans
...
def path_yielder(t, value):
    ...
    def path_helper(currPath, t):
        ...
        if t.branches:
            for branch in t.branches:
                yield from path_helper(copy, branch)
    yield from path_helper([], t)
