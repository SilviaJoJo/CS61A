...
def g_iter(n):
    ...
    if n <= 3:
        return n
    curr1, curr2, curr3 = 1, 2, 3
    index = 4
    while index < n:
        temp = curr1
        curr1 = curr2
        curr2 = curr3
        curr3 = curr2 + 2*curr1 + 3*temp
        index += 1
        pass
    return curr3 + 2*curr2 + 3*curr1
...
def count_change(total):
    ...
    def helper(m, n):
        if m < 0:
            ...
            return 1
        return helper(m - n, n) + helper(m, n//2)
    return helper(total, int(pow(2, int(log(total, 2)))))
...
def make_anonymous_factorial():
    ...
    True
    """
    # I think the point is since we cannot hold a name for fact
    # then we should pass it as an argument
