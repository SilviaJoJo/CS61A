...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    torque1 = length(left(m)) * total_weight(end(left(m)))
    torque2 = length(right(m)) * total_weight(end(right(m)))
    if torque1 == torque2:
...
def totals_tree(m):
    ...
    leftb = totals_tree(end(left(m)))
    rightb = totals_tree(end(right(m)))
    return tree(total_weight(m), [leftb, rightb])
...
def has_path(t, phrase):
    ...
    if label(t) != phrase[0]:
        return False
    elif (not branches(t) or len(phrase) == 1 ) and label(t) == phrase:
        return True
    for b in branches(t):
...
def quadratic(x, a, b, c):
    ...
    def f(t):
        return a * t * t + b * t + c
    middle = (-1) * b / 2 / a
    if middle <= lower_bound(x) or middle >= upper_bound(x):
        return interval(min(f(lower_bound(x)), f(upper_bound(x))), max(f(lower_bound(x)), f(upper_bound(x))))
    return interval(min(f(lower_bound(x)), f(upper_bound(x)), f(middle)), max(f(lower_bound(x)), f(upper_bound(x)), f(middle)))
...
def check_par():
    ...
    r2 = interval(4, 6) # Replace this line!
    return r1, r2
