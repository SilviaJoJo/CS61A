...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    wrongAttempts = []
    def bank(amount, passw0rd):
        "*** YOUR CODE HERE ***"
        nonlocal wrongAttempts
        nonlocal balance
        if len(wrongAttempts) == 3:
            return "Too many incorrect attempts. Attempts: " + str(wrongAttempts)
        elif passw0rd == password:
            if amount > balance:
                return 'Insufficient funds'
            balance = balance - amount
            return balance
        else:
            wrongAttempts.append(passw0rd)
...
def merge(incr_a, incr_b):
    ...
    "*** YOUR CODE HERE ***"
    # note that we cannot simplify the condition to "not" or "if"
    # because the element in list might be 0
