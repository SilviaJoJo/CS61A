...
def cat(password, limit):
    ...
    wrongAttempts = []
    def attempt(digit):
        nonlocal rightAttempts
        nonlocal wrongAttempts
        if len(wrongAttempts) == limit:
            return 'The safe is now inaccessible!'
        if rightAttempts + [digit] == password[:len(rightAttempts) + 1]:
            rightAttempts.append(digit)
            if rightAttempts == password:
                return 'Successfully unlocked!'
        else:
            wrongAttempts.append(digit)
    return attempt
